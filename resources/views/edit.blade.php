        ...
        <form action="{{route('dashboard.update',$plat->id)}}"   method="post" class="border border-gray-200" enctype="multipart/form-data">
            ...
            <div class="mx-4 mt-6 ">
                ...
                <div>
                    ...
                    <select name="uplat_category" id="Ucategory_dishes" class="w-full py-2 bg-gray-50 border border-gray-300 rounded-lg cursor-pointer">
                        @foreach($categorys as $category)
                        <option value="{{$plat->id_category}}" {{$plat->id_category == $category->id ? 'selected' : ''}} >{{$category->name_category}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
